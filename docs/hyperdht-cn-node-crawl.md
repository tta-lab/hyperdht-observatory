...
npm run crawl:dht -- \
  ...
  --frontier 64 \
  --frontier-rounds 3 \
  --output ~/.local/state/hyperdht-observatory
```
...
```sh
npm run report:dht -- \
  --input ~/.local/state/hyperdht-observatory \
  --enrich \
  --stable-min-hours 24 \
...
npm run benchmark:bootstrap -- \
  --recommendations ~/.local/state/hyperdht-observatory/bootstrap-recommendations.json \
  --identity ~/.local/state/kepos-neo/subscriber/client.identity.json \
  --publisher-key <publisher-key> \
  --output ~/.local/state/hyperdht-observatory/benchmark \
  --trials 25 \
  --timeout-ms 30000
```
